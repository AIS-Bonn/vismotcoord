...
void ParticleFilter::setdata(CloudPtr scene, CloudPtr model, int cnt)
{
...
CloudPtr ParticleFilter::getResult()
{
...
void ParticleFilter::initSample()
{
    for (size_t i=0; i<m_particlenum; i++){
...
void ParticleFilter::resample(float varDist, float varAng )
{
...
tracking::ParticleXYZRPY ParticleFilter::sampleWithVar(tracking::ParticleXYZRPY part, float varDist, float varAng)
{
    tracking::ParticleXYZRPY temp;
...
float ParticleFilter::distfeature(VFHSignature308 f1, VFHSignature308 f2)
{
    float dist = 0.f;
...
CloudPtr ParticleFilter::toCloud()
{
    CloudPtr out(new Cloud);
    for (size_t i =0; i<m_particlenum; i++){
        PointT pt;
...
std::vector<float> ParticleFilter::weightGPU(float gridsize, int xnr, int ynr, int znr,
...
std::vector<float> ParticleFilter::getPatchScore(CloudPtr cloud){
...
void ParticleFilter::updateModel(CloudPtr cloud)
{
    m_modelOrig.reset(new Cloud);
